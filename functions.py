...
def loop_train_test(dataID=1, num_list=[], verbose=1, run_times=50, hyper_parameters={}, output_map=False, only_draw_label=False, model_save=False):
    ...
    for run_i in range(run_times):
        ...
        with open('trainlog.txt', 'a') as f:
            f.write(f'run_i: {run_i+1},  accuracy: {OA*100}\n')
    ...
    # 写入mean_oa、mean_aa、mean_kappa
    with open('trainlog.txt', 'a') as f:
        f.write(f'mean_oa: {mean_oa}, mean_aa: {mean_aa}, mean_kappa: {mean_kappa*100}\n')

    with open('trainlog.txt', 'a') as f:
        f.write("各类别精度结果：\n")
        for i in range(n_class):
            class_mean = np.mean(results[i]) * 100
            class_std = np.std(results[i]) * 100
            f.write(f'Class {i+1} mean: {class_mean:.2f}, std: {class_std:.2f}\n')
        f.write("\n")  #
